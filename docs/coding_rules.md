...
- 詳細については[ログ設計](./log_rules.md)を確認してください。
...
- HTTP request では `RequestID` middleware が `request_id` を `gin.Context` と `context.Context` の両方へ設定する
- 認証が必要な request では `AuthMiddleware` が認証済み `user_id` を `gin.Context` と `context.Context` に設定する
- ログ出力時は `logger.WithContext(ctx)` を利用し、`request_id` / `job_id` / `user_id` を自動付与する
