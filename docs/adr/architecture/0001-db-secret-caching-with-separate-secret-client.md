...
- `db_secret` に含まれる値がマスターパスワードであるため、`oswrapper` での `dbSecretClient` 利用は取りやめる
- `MYSQL_USER` / `MYSQL_PASSWORD` は `db_secret` ではなく、アプリ共通 Secret (`appSecretClient`) から取得する
- `DB_HOST` / `DB_PORT` は Secrets Manager の管理対象から外し、タスク定義の環境変数で管理する
- `AGENT_TOKEN_KEY_V1` / `AGENT_TOKEN_SALT` も Secrets Manager の管理対象から外し、タスク定義の環境変数で管理する

この追記により、上記項目に関する `Decision` / `Consequences` の記載は本追記を優先する。
