...
type vendorRecord struct {
	...
	NormalizedName string    `gorm:"column:normalized_name;size:255;not null;uniqueIndex:uni_vendors_normalized_name"`
...
type vendorAliasRecord struct {
	ID              uint      `gorm:"column:id;primaryKey;autoIncrement"`
	VendorID        uint      `gorm:"column:vendor_id;not null;index:idx_vendor_aliases_vendor_id;uniqueIndex:uni_vendor_aliases_vendor_type_value,priority:1"`
	AliasType       string    `gorm:"column:alias_type;size:50;not null;index:idx_vendor_aliases_type_normalized,priority:1"`
	AliasValue      string    `gorm:"column:alias_value;type:text;not null"`
	NormalizedValue string    `gorm:"column:normalized_value;size:255;not null;index:idx_vendor_aliases_type_normalized,priority:2;uniqueIndex:uni_vendor_aliases_vendor_type_value,priority:3"`
	CreatedAt       time.Time `gorm:"column:created_at;not null"`
	UpdatedAt       time.Time `gorm:"column:updated_at;not null"`
}
