...
| パターン | 概要 | 実装箇所 |
| --- | --- | --- |
| interface 経由の呼び出し | 例: `var repo UserRepository; repo.FindByID(id)`。静的に具体実装へ落とし切っていないため、現状は追いません。 | [local_call_graph.go](/home/dev/backend/tools/nplusonecheck/local_call_graph.go), [query_matcher.go](/home/dev/backend/tools/nplusonecheck/query_matcher.go) |
